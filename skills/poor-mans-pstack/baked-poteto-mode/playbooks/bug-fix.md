...
3. Plan the fix. If it crosses a function boundary, run the **architect** skill first. Delegate implementation to a single subagent spawned as your configured `bug-fix` agent (**poor-mans-orchestration** skill; set agents with `/setup-poor-mans-pstack`) with a specific scope, then review the diff yourself. No parallel writers on one fix.
