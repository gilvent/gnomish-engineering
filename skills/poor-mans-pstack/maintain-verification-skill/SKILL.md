...
description: "Periodic pass that keeps a project's verification skill and feature map honest: source coverage per feature, one live session driving every feature, at most one PR of proven corrections. Use for /maintain-verification-skill or \"audit the verify skill\". Budget port of pstack's maintain-verification-skill for a single Claude subscription."
---
...
2. **Source coverage.** For each feature file, answer "how does this user-facing feature work?" from source, flag likely doc drift with citations, and produce one concise live-verification recipe (shape: feature summary / source entry points / likely drift or none / one recipe). Three or fewer features: do this inline. More: spawn `maintain-verification readers`-tier subagents per the **poor-mans-orchestration** skill — read it first — batching the feature files across at most 2-3 read-only subagents launched in one message, each with its own output path. Readers never drive the app and never edit files; upstream pstack runs one reader per feature, so note the batching in the run notes.
